...
// Public news feeds we scan for fresh Canadian scam warnings.
const FEEDS = [
...
export const Route = createFileRoute("/api/public/hooks/refresh-scam-alerts")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        // The scheduler authenticates with the project's public key.
        const provided =
        ...
        if (headlines.length === 0) {
          return Response.json({ ok: true, added: 0, note: "No headlines available right now" });
        }

        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

        // Skip anything we've already stored (approved, pending or rejected).
        ...
        if (fresh.length === 0) {
          return Response.json({ ok: true, added: 0, note: "Nothing new since last run" });
        }

        // 2. Turn the headlines into plain-English alerts for seniors.
        const iconList = Object.keys(ALERT_ICONS).join(", ");
        ...
        if (!aiRes.ok) {
          ...
          return Response.json({ error: "AI request failed", status: aiRes.status }, { status: 502 });
        }
        ...
        if (!args) return Response.json({ ok: true, added: 0, note: "AI returned nothing usable" });

        let alerts: Record<string, unknown>[] = [];
        try {
          alerts = (JSON.parse(args).alerts ?? []) as Record<string, unknown>[];
        } catch {
          return Response.json({ ok: true, added: 0, note: "AI returned unreadable output" });
        }
        ...
        if (rows.length === 0) return Response.json({ ok: true, added: 0, note: "No alerts worth publishing" });
        ...
        return Response.json({ ok: true, added: rows.length, scanned: fresh.length });
      },
    },
  },
});
